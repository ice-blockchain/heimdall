...
func (t *tokenAnalytics) GetTokenPricing(ctx context.Context, externalAddress string, tradeType TradeType) (amount uint64, amountUsd float64, err error) {
	...
	resAmount, err := t.bondingCurve.Pricing(ctx, common.HexToAddress(result.BaseToken), common.HexToAddress(result.ContractAddress), big.NewInt(100), tradeType == TradeTypeSell)
	if err != nil {
