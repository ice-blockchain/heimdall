...
func (t *tokenAnalytics) GetLatestTrades(ctx context.Context, externalAddress string, limit, offset uint64, startFrom *time.Time) ([]*Trade, time.Time, error) {
	...
	sql := fmt.Sprintf(`
		...
		LEFT JOIN user_token_positions utp ON utp.external_address = token_swaps.external_address AND utp.master_pubkey = holder.master_pubkey
	...
	for i := range swaps {
		...
		balanceWeiBigInt := new(big.Int)
		if _, ok := balanceWeiBigInt.SetString(swaps[i].Balance, 10); !ok {
