...
ALTER TABLE tokens_featured DROP CONSTRAINT IF EXISTS tokens_featured_external_address_fkey;

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tokens_platform_holders_count') THEN
        ALTER TABLE tokens_platform_holders_count DROP CONSTRAINT IF EXISTS tokens_platform_holders_count_external_address_fkey;
    END IF;
END $$;

ALTER TABLE uniswap_pools DROP CONSTRAINT IF EXISTS uniswap_pools_token_address_fkey;

ALTER TABLE tokens DROP CONSTRAINT IF EXISTS tokens_pkey;
ALTER TABLE tokens ADD PRIMARY KEY (external_address);
CREATE UNIQUE INDEX IF NOT EXISTS idx_tokens_contract_address_unique ON tokens (contract_address);
...
ALTER TABLE uniswap_pools 
    ADD CONSTRAINT uniswap_pools_token_address_fkey 
    FOREIGN KEY (token_address) REFERENCES tokens(contract_address) ON DELETE CASCADE;

ALTER TABLE tokens_featured 
    ADD CONSTRAINT tokens_featured_external_address_fkey 
    FOREIGN KEY (external_address) REFERENCES tokens(external_address) ON DELETE CASCADE;

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tokens_platform_holders_count') THEN
        ALTER TABLE tokens_platform_holders_count 
            ADD CONSTRAINT tokens_platform_holders_count_external_address_fkey 
            FOREIGN KEY (external_address) REFERENCES tokens(external_address) ON DELETE CASCADE;
    END IF;
END $$;
