...
ALTER TABLE token_swaps ADD COLUMN IF NOT EXISTS curve_price_usd usd_amount DEFAULT 0;
ALTER TABLE token_swaps ADD COLUMN IF NOT EXISTS notified_at TIMESTAMP;
...
DROP TRIGGER IF EXISTS token_swap_update_trigger ON token_swaps;
CREATE TRIGGER token_swap_update_trigger
    AFTER UPDATE OF curve_price_usd ON token_swaps
    FOR EACH ROW
    WHEN (
        OLD.curve_price_usd IS DISTINCT FROM NEW.curve_price_usd AND
        NEW.curve_price_usd > 0
        )
EXECUTE FUNCTION notify_token_swap_update();
