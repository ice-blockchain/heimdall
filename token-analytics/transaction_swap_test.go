...
func TestContentPoolPosition(t *testing.T) {
	...
	t.Run("content token pool is registered when swap of content token occurs", func(t *testing.T) {
		...
		require.Eventually(t, func() bool {
			score, err := ta.processedDataDB.ZScore(ctx, keyUserPositionOfToken(ionConnectAddr), masterPubkey).Result()
...
func TestOnSwap(t *testing.T) {
	...
	t.Run("processes_double_fat_address_swap_successfully", func(t *testing.T) {
		...
		require.Eventually(t, func() bool {
			score, err := ta.processedDataDB.ZScore(ctx, keyUserPositionOfToken(contentExternalAddr), buyerPubkey).Result()
			return err == nil && score > 0
	...
	t.Run("full_flow_double_fat_address_content_token", func(t *testing.T) {
		...
		// Verify second buy - RPC mock always returns 1 token balance
		initialBalance, err := ta.processedDataDB.ZScore(ctx, keyUserPositionOfToken(contentExternalAddr), buyerPubkey).Result()
		require.NoError(t, err)
		require.Equal(t, 1.0, initialBalance, "Balance should be 1.0 (mock RPC returns fixed 1 token)")
		...
		// Verify sell - RPC mock always returns 1 token balance
		finalBalance, err := ta.processedDataDB.ZScore(ctx, keyUserPositionOfToken(contentExternalAddr), buyerPubkey).Result()
		require.NoError(t, err)
		require.Equal(t, 1.0, finalBalance)
...
func TestExtractAllTokensFromFatAddress(t *testing.T) {
	...
	t.Run("single_fat_address", func(t *testing.T) {
		...
		require.Equal(t, "test_external_addr", tokens[0].ExternalAddress)
	...
	t.Run("double_fat_address", func(t *testing.T) {
		...
		require.Equal(t, "0:creator_pubkey:", tokens[0].ExternalAddress, "First token should be creator")
		require.Equal(t, TokenTypeProfile, tokens[0].Type, "First token should be profile")
		require.Equal(t, TokenTypePost, tokens[1].Type, "Second token should be post")
		require.Equal(t, "30175:creator_pubkey:post123", tokens[1].ExternalAddress, "Second token should be content")
		require.Equal(t, common.Address{}, creator)
		require.Equal(t, common.Address{}, affiliate)
	})
	...
	t.Run("with_creator_and_affiliate", func(t *testing.T) {
		...
		require.Equal(t, "test_addr", tokens[0].ExternalAddress)
...
func TestExtractAllTokensFromFatAddress_RecommendedUsage(t *testing.T) {
	...
	t.Run("single_swap_example", func(t *testing.T) {
		...
		require.Equal(t, "test_external", tokens[0].ExternalAddress)
		require.Equal(t, common.Address{}, creator)
		require.Equal(t, common.Address{}, affiliate)
		require.Equal(t, TokenTypeProfile, tokens[0].Type)
	})
	...
	t.Run("double_swap_example", func(t *testing.T) {
		...
		require.Equal(t, "0:creator:", tokens[0].ExternalAddress, "First token is creator")
		require.Equal(t, "30175:creator:post", tokens[1].ExternalAddress, "Second token is content")
		require.Equal(t, TokenTypeProfile, tokens[0].Type, "First token is profile")
		require.Equal(t, TokenTypePost, tokens[1].Type, "Second token is post")
	})
}
