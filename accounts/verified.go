	...
	return errors.Wrap(storage.DoInTransaction(ctx, a.db, func(conn storage.QueryExecer) error {
		query := `
			...
			RETURNING master_pubkey, 
				COALESCE(
					(SELECT json_agg(x) FROM (SELECT userurl as url, relay_type as "type" FROM ion_connect_relays join unnest(ion_connect_relays) AS t(userurl) ON url = userurl OR url = replace(userurl, ':4443','')) x),
					'[]'::json
