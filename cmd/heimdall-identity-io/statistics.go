...
func (s *service) validateEvent(ctx context.Context, event *model.Event) error {
	...
	if err := s.validation.Validate(ctx, model.Events{event}, validation.RuleWithSkipRootContentNFTCollectionsValidation(), validation.RuleWithSkipRootContentReplyValidation()); err != nil {
